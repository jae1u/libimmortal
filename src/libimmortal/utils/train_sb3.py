...
from libimmortal.samples.reward import ImmortalRewardShaper
from libimmortal.samples.obs_builder import BasicObsBuilder, ArrowObsBuilder
...
class GymnasiumWrapper(gym.Env):
    def __init__(self, game_path, port, time_scale=2.0, seed=42, max_steps=2000, obs_builder=None):
        ...
        # Observation space
        # graphic: (3, 90, 160), vector: (103,)
        self.observation_space = spaces.Dict({
            'graphic': spaces.Box(low=0, high=255, shape=(3, 90, 160), dtype=np.uint8),
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.obs_builder:
            obs = self.obs_builder.build(obs)
            vector_obs = obs.get('vector', obs.get('image', np.zeros(103)))
            if vector_obs.shape[0] >= 103:
                self.reward_shaper.reset(vector_obs[:103])
            else:
                self.reward_shaper.reset(vector_obs)
        else:
            graphic_obs, vector_obs = self._parse_observation(obs)
            self.reward_shaper.reset(vector_obs)
            obs = {'graphic': graphic_obs, 'vector': vector_obs}
    ...
    def step(self, action):
        ...
        if self.obs_builder:
            obs_dict = self.obs_builder.build(obs)
            vector_obs = obs_dict.get('vector', obs_dict.get('image', np.zeros(103)))
            if vector_obs.shape[0] >= 103:
            ...
            graphic_obs, vector_obs = self._parse_observation(obs)
            vector_for_reward = vector_obs
            obs_dict = {'graphic': graphic_obs, 'vector': vector_obs}
        ...
        prev_enemies = self.reward_shaper.prev_enemy_count if hasattr(self.reward_shaper, 'prev_enemy_count') else 0
        current_enemies = self.reward_shaper._count_enemies(vector_for_reward)
        if prev_enemies > current_enemies:
            self.enemies_killed_this_ep += (prev_enemies - current_enemies)
        
        truncated = self.current_step >= self.max_steps
        if truncated and not done:
            info['TimeLimit.truncated'] = True
        
        shaped_reward = self.reward_shaper.compute_reward(vector_for_reward, reward, done, truncated)
...
def main():
    ...
    print(f"Device: {'cuda' if torch.cuda.is_available() else 'cpu'} | Total: {args.total_timesteps:,}")
